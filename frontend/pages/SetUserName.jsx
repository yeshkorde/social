...
const {userData} = useUserContext()
  const ChickUserNameAvalable = async (e) => {
    try {
      if (!e.target.value) {
        seterror("");
        setdata("")
      ...
      if (e.target.value.length <= 5) {
        return seterror("username must me five charecter long");
      }
      seterror("");
      ...
      const res = await axios.post(
        ...
        }/api/user/chickUserName?q=${userName}`,
        "_",
        {
      ...
      console.log("some thing want wrong to chicuser", error.message);
    }
  };
...
if(userData.userName){
  ...
  return (
    <div className="h-[690px] w-screen bg-white flex items-center justify-center">
      <div className=" w-1/3 rounded-xl  bg-white shadow-[3px_3px_30px] shadow-[#e6e6e6]  dark:border  dark:bg-black dark:shadow-[#000] p-10 flex flex-col  items-center">
        <h1 className="text-center font-semibold text-xl">Set your UserName</h1>
        <div className="flex flex-col w-full mt-10">
          <label htmlFor="usename" className="ml-2 mb-2 font-semibold">
            UserName
          </label>
          <input
            type="text"
            className="h-10 w-full border border-[#cecece]  rounded-xl outline-none px-6"
            onChange={(e) => ChickUserNameAvalable(e)}
          />
          <div className="flex justify-between">
            {error ? (
              <p className="text-[12px] font-semibold ml-2 text-red-600">{error}</p>
            ) : (
              <p className="text-[12px] ml-2 font-semibold text-green-600">{data}</p>
            )}
            {isLoding ? (
              <svg
                width="20"
        ...
        {data ? (
          <button className="px-10 w-full h-10 text-white bg-red-500 mt-10 rounded-xl font-semibold">
            Set userName
          </button>
        ) : (
          ""
